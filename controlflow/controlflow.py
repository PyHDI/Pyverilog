...
# controlflow.py
#
# Control-flow analyzer from Verilog Definitions with Pyverilog
#
# For visualization of control-flow, graphviz and pygraphviz are required
#
...
from pyverilog.subset.subset import VerilogSubset
from pyverilog.tree_walker.tree_walker import VerilogTreeWalker    

class VerilogControlflow(VerilogSubset):
    def __init__(self, topmodule, terms, binddict,
        ...
        self.treewalker = VerilogTreeWalker(topmodule, terms, binddict, 
                                            resolved_terms, resolved_binddict, constlist)
    ...
    def getFiniteStateMachine(self, termname, funcdict):
        ...
        for condlist, func in sorted(funcdict.items(), key=lambda x:len(x[0])):
            ...
            node = state_transition.walkCondlist(condlist, termname, width)
            if node is None: continue
            statenode_list = node.nodelist if isinstance(node, state_transition.StateNodeList) else [node,]
    ...
    def makeTree(self, termname):
        ...
        tree = tree_reorder.reorder(tree)
        tree = self.optimizer.optimize(tree)
        tree = tree_replace.replaceUndefined(tree, termname)
        return tree
...
if __name__ == '__main__':
    ...
    INFO = "Control-flow analyzer for Verilog definitions with Pyverilog"
    VERSION = pyverilog.utils.version.VERSION
    USAGE = "Usage: python controlflow.py -t TOPMODULE file ..."
    ...
    controlflow = VerilogControlflow(options.topmodule, terms, binddict, resolved_terms, resolved_binddict, constlist)
    fsms = controlflow.getFiniteStateMachines()
