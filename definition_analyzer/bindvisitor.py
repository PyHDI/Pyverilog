...
# bindvisitor.py
# 
# Verilog binding visitor with Pyverilog
#
...
class BindVisitor(NodeVisitor):
    ...
    def getTree(self, node, scope):
        ...
        if not self.noreorder: tree = tree_reorder.reorder(tree)
        return tree
    ...
    def resolveBlockingAssign(self, tree, scope):
        ...
        if isinstance(tree, DFBranch):
            ...
            if isinstance(condnode, DFBranch):
                return tree_reorder.insertBranch(condnode, truenode, falsenode)
        ...
        if isinstance(tree, DFOperator):
            ...
            for r in resolvednodes:
                if isinstance(r, DFBranch):
                    return tree_reorder.insertOpList(resolvednodes, tree.operator)
        ...
        if isinstance(tree, DFConcat):
            ...
            for r in resolvednodes:
                if isinstance(r, DFBranch):
                    return tree_reorder.insertConcat(resolvednodes)
        ...
        if isinstance(tree, DFPartselect):
            ...
            if isinstance(resolved_var, DFBranch):
                return tree_reorder.insertPartselect(resolved_var, 
        ...
        if isinstance(tree, DFPointer):
            ...
            resolved_var = self.resolveBlockingAssign(tree.var, scope)
            if isinstance(resolved_var, DFBranch):
                return tree_reorder.insertPointer(resolved_var, resolved_ptr)
    ...
    def getDestinations(self, left, scope):
        ...
        for name, msb, lsb, ptr in reversed(dst):
            ...
            diff = tree_reorder.reorder( DFOperator((msb, lsb), 'Minus') )
            part_lsb = part_offset 
            part_msb = tree_reorder.reorder(DFOperator((part_offset, diff), 'Plus'))
            part_offset = tree_reorder.reorder(
    ...
    def setNonblockingAssign(self, name, dst, raw_tree, msb, lsb, ptr,
        ...
        tree = raw_tree
        if len(dst) > 1:
            tree = tree_reorder.reorder(DFPartselect(raw_tree, part_msb, part_lsb))
    ...
    def setRenamedTree(self, renamed_dst, raw_tree, alwaysinfo):
        for name, msb, lsb, ptr, part_msb, part_lsb in renamed_dst:
            tree = raw_tree
            if len(renamed_dst) > 1: tree = tree_reorder.reorder(
    ...
    def makeBind(self, name, msb, lsb, ptr, part_msb, part_lsb, 
        ...
        tree = tree_reorder.reorder(
            self.appendBranchTree(current_tree, match_flowlist, add_tree))
        
        if num_dst > 1: tree = tree_reorder.reorder(
            DFPartselect(tree, part_msb, part_lsb))
