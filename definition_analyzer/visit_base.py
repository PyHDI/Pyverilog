...
#map_key   = lambda f,d: dict([ (f(k),v) for k,v in d.items() ])
#map_value = lambda f,d: dict([ (k,f(v)) for k,v in d.items() ])
...
class FrameTable(object):
    ...
    def getLabelKey(self, name):
        ret = ''
        if self.isModule(): ret += 'md_'
        if self.isGenerate(): ret += 'ge_'
        if self.isAlways(): ret += 'al_'
        if self.isInitial(): ret += 'in_'
        if self.isFunctioncall(): ret += 'fc_'
        if self.isTaskcall(): ret += 'tc_'
        return ret + name
    ...
    def searchConstantDefinition(self, key, name):
        ...
        if self.dict[key].isModule(): return None, None
