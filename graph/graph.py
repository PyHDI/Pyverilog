...
# graph.py
#
# Graph generator for Verilog definition tree with Pyverilog
#
# This class requires pygraphviz and graphviz
#
...
class VerilogGraph(object):
    ...
    def __init__(self, topmodule,
        ...
        self.treewalker = VerilogTreeWalker(
            self.topmodule, self.terms, self.binddict, self.resolved_terms, self.resolved_binddict, constlist)
    ...
    def generate(self, signalname, identical=False, walk=True, step=1, reorder=False, delay=False):
        ...
        if walk: 
            ...
            if reorder: tree = tree_reorder.reorder(tree)            
...
if __name__ == '__main__':
    ...
    INFO = "Graph generator for Verilog tree with Pyverilog"
    VERSION = pyverilog.utils.version.VERSION
    USAGE = "Usage: python graph.py -t TOPMODULE -s TARGETSIGNAL file ..."
    ...
    veriloggraph = VerilogGraph(options.topmodule, terms, binddict, resolved_terms, resolved_binddict, constlist, options.outputfile)

    for target in options.searchtarget:
        veriloggraph.generate(target, walk=options.walk, identical=options.identical, step=options.step, reorder=options.reorder, delay=options.delay)
    veriloggraph.draw()
