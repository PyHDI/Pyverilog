...
def notStateNodeList(node):
    ...
    if isStateNodeList(node):
        ...
        ret_node = None
        for n in new_nodes:
            if ret_node is None: ret_node = n
            else: ret_node = andStateNodeList(ret_node, n)
        return ret_node
    return None
...
def andStateNodeList(lnode, rnode):
    if lnode is None and rnode is None: return None
    if lnode is None: return rnode
    if rnode is None: return lnode
...
def notStateNode(node):
    ...
    if node.transcond is None: return a
    new_transcond = _not_transcond(node.transcond)
...
def andStateNode(sna, snb):
    if sna is None and snb is None: return None
    if sna is None: return snb
    if snb is None: return sna
...
class StateNode(object):
    ...
    def __repr__(self):
        ...
        ret = ret[:-1]
        ret += ', cond:'
        if isinstance(self.transcond, DFNode): ret += self.transcond.tocode()
        else: ret += str(self.transcond)
        return ret
