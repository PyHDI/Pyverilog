...
class VerilogParser(PLYParser):
    ...
    def p_pragma_assign(self, p):
        ...
        p[0] = Pragma( PragmaEntry(p[3], p[5]) )
    ...
    def p_param(self, p):
        ...
        paramlist = [ Parameter(rname, rvalue) for rname, rvalue in p[2] ]
        p[0] = Decl( tuple(paramlist) )

    def p_param_width(self, p):
        'param : PARAMETER width param_substitution_list COMMA'
        paramlist = [ Parameter(rname, rvalue, p[2]) for rname, rvalue in p[3] ]
        p[0] = Decl( tuple(paramlist) )

    def p_param_integer(self, p):
        'param : PARAMETER INTEGER param_substitution_list COMMA'
        paramlist = [ Parameter(rname, rvalue) for rname, rvalue in p[3] ]
        p[0] = Decl( tuple(paramlist) )
        ...
        if isinstance(p[3], str):
            ...
            for r in reversed(p[1]):
                if isinstance(r.first,  Input):
                    t = Ioport(Input(name=p[3],width=r.first.width))
                    break
                if isinstance(r.first, Output) and r.second is None:
                    t = Ioport(Output(name=p[3],width=r.first.width))
                    break
                ...
                if isinstance(r.first,  Inout):
                    t = Ioport(Inout(name=p[3],width=r.first.width))
                    break
            p[0] = p[1] + (t,)
        else:
            p[0] = p[1] + (p[3],)
    ...
    def ioport_create(self, sigtypes, name, width=None):
        ...
        return Ioport(first, second)

    def ioport_typecheck(self, sigtypes):
    ...
    def p_ioport(self, p):
        ...
        p[0] = self.ioport_create(p[1], p[2])

    def p_ioport_width(self, p):
        'ioport : sigtypes width portname'
        p[0] = self.ioport_create(p[1], p[3], width=p[2])
    ...
    def decl_create(self, sigtypes, name, width=None, length=None):
        ...
        if 'signed' in sigtypes:
            signed = True
        if 'input' in sigtypes:
            decls.append( Input(name=name, width=width, signed=signed) )
        if 'output' in sigtypes:
            decls.append( Output(name=name, width=width, signed=signed) )
        if 'inout' in sigtypes:
            decls.append( Inout(name=name, width=width, signed=signed) )
        if 'wire' in sigtypes:
            if length:
                decls.append( WireArray(name=name, width=width,
                                        signed=signed, length=length) )
            else:
                decls.append( Wire(name=name, width=width, signed=signed) )
        if 'reg' in sigtypes:
            if length:
    ...
    def p_decl(self, p):
        ...
        for rname, rlength in p[2]:
            decllist.extend( self.decl_create(p[1], rname, length=rlength) )
        p[0] = Decl( tuple(decllist) )

    def p_decl_width(self, p):
        'decl : sigtypes width declnamelist SEMICOLON'
        decllist = []
        for rname, rlength in p[3]:
            decllist.extend( self.decl_create(p[1], rname, width=p[2], length=rlength) )
        p[0] = Decl( tuple(decllist) )
    ...
    def declassign_typecheck(self, sigtypes):
        if len(sigtypes) == 1 and 'signed' in sigtypes:
            raise ParseError("Syntax Error")
    ...
    def p_realdecl(self, p):
        ...
        reallist = [ Real(p[1], Width(msb=IntConst('31'),lsb=IntConst('0'))) for r in p[2] ]
        p[0] = Decl( tuple(reallist) )
    ...
    def p_blocking_substitution(self, p):
        ...
        p[0] = BlockingSubstitution(p[2], p[5], p[1], p[4])
    ...
    def p_nonblocking_substitution(self, p):
        ...
        p[0] = NonblockingSubstitution(p[2], p[5], p[1], p[4])
    ...
    def p_for_statement(self, p):
        ...
        p[0] = ForStatement(p[3], p[4], p[5], p[7])
    ...
    def p_genvar(self, p):
        'genvar : ID'
        p[0] = Genvar(name=p[1], width=Width(msb=IntConst('31'),lsb=IntConst('0')))

    def p_generate(self, p):
        'generate : GENERATE generate_items ENDGENERATE'
        p[0] = GenerateStatement(p[2])
    ...
    def p_function_nowidth(self, p):
        ...
        p[0] = Function(p[2], Width(IntConst('0'), IntConst('0')), p[4])
    ...
    def p_single_statement_delays(self, p):
        ...
        p[0] = SingleStatement(DelayStatement(p[2]))
