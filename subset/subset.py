...
# subset.py
#
# Subset generator from Verilog definitions with Pyverilog
#
...
class VerilogSubset(VerilogTreeConstructor):
    def __init__(self, topmodule, terms, binddict,
                 resolved_terms, resolved_binddict, constlist):
        VerilogTreeConstructor.__init__(self, topmodule, terms, binddict,
                                        resolved_terms, resolved_binddict, constlist)
        self.clock_name = 'CLK'
...
if __name__ == '__main__':
    ...
    INFO = "Subset generator from Verilog tree definitions with Pyverilog"
    ...
    verilogsubset = VerilogSubset(options.topmodule, terms, binddict, resolved_terms, resolved_binddict, constlist)

    verilogsubset.set_clock_info(options.clockname, options.clockedge)
    verilogsubset.set_reset_info(options.resetname, options.resetedge)

    sub_binds, sub_terms = verilogsubset.getBindSourceSubset(options.searchtarget)
    terms, parameter, assign, always_clockedge, always_combination = verilogsubset.getSubset(options.searchtarget)
