...
# tree_constructor.py
# 
# Definition Tree Constructor with Pyverilog
#
...
class VerilogTreeConstructor(object):
...
if __name__ == '__main__':
    ...
    INFO = "Verilog tree constructor with Pyverilog"
    VERSION = pyverilog.utils.version.VERSION
    USAGE = "Usage: python tree_constructor.py -t TOPMODULE file ..."
    ...
    verilogtreeconstructor = VerilogTreeConstructor(options.topmodule, terms, binddict, resolved_terms, resolved_binddict, constlist)

    for target in options.searchtarget:
        termname = util.toTermname(target)
        tree = verilogtreeconstructor.getTree(termname)
